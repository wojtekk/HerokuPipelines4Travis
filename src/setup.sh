...
if [ -z "$IN_HEROKU_APPNAME_STAGE" ]; then
  IN_HEROKU_APPNAME_STAGE="$IN_HEROKU_APPNAME_PRODUCTION-stage"
fi;
sed -i "s|__HP4T_HEROKU_APPNAME_STAGE__|$IN_HEROKU_APPNAME_STAGE|" $HP4T_PROJECT_DIR/.travis.yml;
